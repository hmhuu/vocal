...
ax.set_title("Vocal Analyzer (Advanced Config)")
...
class ConfigPopup:
    ...
    def open(self, event):
        ...
        self.fig = plt.figure("Advanced Settings", figsize=(5, 3.5))
    ...
    def apply(self, event):
        ...
        try:
            new_int = float(self.txt_int.text)
            new_hz = float(self.txt_hz.text)
            
            # Points 제한 로직 (최대 300)
            requested_pts = int(float(self.txt_pts.text))
            new_pts = min(requested_pts, 300)
            ...
            # 배열 재계산 (data_points가 바뀌었으므로 반드시 필요)
            x_raw = np.linspace(0, target_hz, data_points)
            x_smooth = np.linspace(0, target_hz, SMOOTH_FACTOR)
            noise_floor = np.zeros(data_points)
            ...
            plt.close(self.fig)
            print(f"적용 완료: Hz={target_hz}, Points={data_points}")
        except ValueError:
            pass
...
# --- 하단 버튼 배치 ---
ax_config = plt.axes([0.05, 0.05, 0.12, 0.05])
btn_config = Button(ax_config, 'Configure', color='lightgray')
btn_config.on_clicked(config_handler.open)

ax_box = plt.axes([0.22, 0.05, 0.08, 0.05])
text_box = TextBox(ax_box, 'Note: ', initial='C4')
ax_harm = plt.axes([0.31, 0.05, 0.1, 0.05])
